...
def expand_placeholders(obj, home_dir: str, workspace_dir: str = "", username: str = ""):
    """Recursively replace {HOME}, {WORKSPACE}, and {USER} placeholders in strings."""
    if not username:
        try:
            import getpass
    ...
    if isinstance(obj, str):
        res = obj.replace("{HOME}", home_dir)
        if workspace_dir:
            res = res.replace("{WORKSPACE}", workspace_dir)
        if username:
            res = res.replace("{USER}", username)
        return res
    elif isinstance(obj, dict):
        return {k: expand_placeholders(v, home_dir, workspace_dir, username) for k, v in obj.items()}
    elif isinstance(obj, list):
        return [expand_placeholders(item, home_dir, workspace_dir, username) for item in obj]
    return obj
...
def sync_local_config(template_path: str, workspace_dir: str = "", dry_run: bool = False) -> bool:
    ...
    # Expand cross-platform {HOME}, {WORKSPACE}, and {USER} placeholders
    template_data = expand_placeholders(template_raw, home_dir, workspace_dir=workspace_dir)
...
WORKSPACE_CONF_FILE = os.path.join(get_home_dir(), ".gemini", "antigravity-suite.conf")


def resolve_workspace_dir(parent_workspace: str) -> str:
    """Resolve the workspace root directory using a priority chain:
    1. ANTIGRAVITY_WORKSPACE / AGY_WORKSPACE environment variable
    2. ~/.gemini/antigravity-suite.conf persisted config
    3. Parent directory of the suite repo (auto-detected, covers all drive letters and OS paths)
    """
    # 1. Environment variable (highest priority, useful for CI, scripts, or non-standard setups)
    env_workspace = os.environ.get("ANTIGRAVITY_WORKSPACE") or os.environ.get("AGY_WORKSPACE")
    if env_workspace and os.path.isdir(env_workspace):
        return os.path.abspath(env_workspace)

    # 2. Persisted config file at ~/.gemini/antigravity-suite.conf
    if os.path.isfile(WORKSPACE_CONF_FILE):
        try:
            with open(WORKSPACE_CONF_FILE, "r", encoding="utf-8") as f:
                conf = json.load(f)
            saved = conf.get("workspace_dir", "")
            if saved and os.path.isdir(saved):
                return os.path.abspath(saved)
        except Exception:
            pass

    # 3. Auto-detect from repo location (works regardless of drive letter, OS, or path depth)
    return parent_workspace


def save_workspace_conf(workspace_dir: str) -> None:
    """Persist workspace directory to ~/.gemini/antigravity-suite.conf."""
    try:
        os.makedirs(os.path.dirname(WORKSPACE_CONF_FILE), exist_ok=True)
        conf = {}
        if os.path.isfile(WORKSPACE_CONF_FILE):
            with open(WORKSPACE_CONF_FILE, "r", encoding="utf-8") as f:
                conf = json.load(f)
        conf["workspace_dir"] = workspace_dir
        with open(WORKSPACE_CONF_FILE, "w", encoding="utf-8") as f:
...
def main():
    ...
    default_workspace = resolve_workspace_dir(parent_workspace)
    ...
    parser.add_argument(
        ...
        help="Path to workspace root directory for --workspace-sync and template deployment",
    )
    parser.add_argument(
        "--migrate",
        action="store_true",
    ...
    parser.add_argument(
        ...
        metavar="DIR",
        default=None,
        help="Pin a custom workspace root directory to ~/.gemini/antigravity-suite.conf for future runs",
    )
    args = parser.parse_args()

    # Persist workspace if explicitly set by user
    if args.set_workspace:
        ws_path = os.path.abspath(args.set_workspace)
        if not os.path.isdir(ws_path):
            print(f"[-] Error: workspace directory does not exist: {ws_path}", file=sys.stderr)
            sys.exit(1)
        save_workspace_conf(ws_path)
        print(f"[+] Workspace directory pinned: {ws_path}")
        print(f"    (Saved to {WORKSPACE_CONF_FILE})")
        return
